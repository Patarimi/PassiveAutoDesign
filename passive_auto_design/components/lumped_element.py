...
class LumpedElement(metaclass=abc.ABCMeta):
    ...
    #TODO enable sub-part in the form of "part."
    @functools.lru_cache()
    def __cost(self, x_value : float, x_key : str, y_key : str, y_value : float):
        ...
        if x_key.find(".") > 0:
            comp, ref = x_key.split(".", 1)
            self.par[comp].par.update({ref: x_value})
            return abs(self.par[comp].calc_ref_value() - self.par[comp].par[ref])
