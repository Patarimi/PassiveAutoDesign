...
class Substrate(HydraHeadApp):
    def run(self):
        ...
        with st.form(key="substrate_definition"):
            ...
            min_w = col[1].number_input("Min. Width")
            max_w = col[2].number_input("Max. Width")
            gap = col[3].number_input("Gap between track")
            submitted = st.form_submit_button("Add layer")

        if submitted:
            substrate.add_layer(
                sub.Layer(layer_name, 3e-6, sub.COPPER, sub.SILICON_OXYDE)
            )
            substrate.sub[substrate.get_index_of(layer_name)].set_rules(
                min_w, max_w, gap
            )
