...
with col1.form(key="Transformer Parameters"):
    ...
    width_min = (
        st.number_input(label="Width (µm)", value=3.0, min_value=0.0, step=0.01) * 1e-6
    )
    gap = st.number_input(label="Gap (µm)", value=1.5, min_value=0.0, step=0.01) * 1e-6
    ...
    dist_g = st.number_input(label="Distance to the ground plane (µm)", value=3.0)
    st.form_submit_button(label="Compute")
